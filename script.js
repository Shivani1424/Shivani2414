...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const getStartedBtn = document.querySelector("a button"); // Select the button inside the <a> tag

    // Disable the button initially
    getStartedBtn.disabled = true;
    getStartedBtn.style.opacity = "0.5"; // Make button look disabled

    function validateInputs() {
        if (emailInput.value.trim() !== "" && passwordInput.value.trim() !== "") {
            getStartedBtn.disabled = false;
            getStartedBtn.style.opacity = "1"; // Restore normal look
        } else {
            getStartedBtn.disabled = true;
            getStartedBtn.style.opacity = "0.5"; // Keep disabled look
        }
    }
    ...
    // Prevent redirection if fields are empty
